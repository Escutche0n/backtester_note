...
3. `docs/architecture/overview.v1.md`
4. `docs/project_state.md` —— 当前态聚合视图（Phase 进度 / 决议簿 / 红线状态 / 待答问题 / 下一步）
5. `docs/design/backtester-note/README.md` + 当前任务涉及的 `.html` / `.jsx` / `bn-tokens.css`
6. `docs/worklog/` 里自己层最近 2 条 + 对方层最近 1 条 session log
7. 本次要改的 `docs/contracts/*` 对应契约

**按当次任务再读：**

8. `docs/algorithms/` 里相关口径
9. 旧 iOS 客户端 `/Users/elvischen/Developer/investment app/`（参考而非迁移）
10. 后端本地 clone `/Users/elvischen/Developer/backtester_backend`
...
**干活的一方**：收工必写 `docs/worklog/YYYY-MM-DD_<layer>_<slug>.md`，`<layer>` 为 `ios` 或 `be`。模板见 `docs/worklog/_template.md`。**同一 commit 还须更新 `docs/project_state.md`**：§1 状态格、§2 新决议、§4 新问题、§5 下一步、§0 last-updated 日期与更新者。详见 `project_state.md` §6 维护规则。
...
- **单代码文件 ≤ 450 行**（含注释，不含空行）。触到 450 必须拆，没有商量。
- **只限代码，不限 markdown**：`.swift` / `.py` / `.ts` / `.js` 等代码文件守 450；`docs/` 下的 `.md` 不守，文档拆碎反而难读。
- **配置 / 数据文件豁免**：`Package.swift`、`project.yml`、`Info.plist`、`*.entitlements`、JSON fixture、自动生成的 schema —— 这些不是手写代码，不计行数。
- **一个文件一个语义对象**：一个 view / 一个 service / 一个算法函数族 / 一个契约对象。不要把多个 view、多个不相关的 service / 函数堆在同一文件里。
- **拆分方向「按职责」而不是「按行数硬切」**：临到 450 不是机械 split，而是想清楚这个文件承担了几件事，按语义拆。如果实在切不开（罕见），写进 worklog 的 `## Ideas`，下个 commit 重构 —— 但当前 commit 仍必须拆到 ≤ 450。
- **目录即分工**：文件落在谁的目录（见上方"并行模式"表）就由谁负责。新增模块前先确认目录归属，不要跨界落文件。
